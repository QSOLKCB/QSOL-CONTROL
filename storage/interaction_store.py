...
RUN_PROTOCOL = "qsol-control-interaction/1"
...
def lattice_address(
    ...
    """Return the canonical top-level 3x3x3 lattice address."""
    try:
...
class InteractionStore:
    ...
    def create_run(
        ...
        *,
        ...
        nexus_refs: Iterable[str] = (),
        model_state_refs: Iterable[str] = (),
        replayability: str = "R0",
    ) -> dict[str, Any]:
        ...
        question_sha256 = _sha256_hex(question_bytes)
        question_lattice = lattice_address("question", "derived", "current")

        oracle_values = list(oracle_refs)
        nexus_values = list(nexus_refs)
        if any(not isinstance(value, str) or not value for value in oracle_values + nexus_values):
            raise StorageError("oracle_refs and nexus_refs must be non-empty strings")
        oracle = sorted(set(oracle_values), key=lambda value: value.encode("utf-8"))
        nexus = sorted(set(nexus_values), key=lambda value: value.encode("utf-8"))
        ...
        payload: dict[str, Any] = {
            ...
            "question": {
                ...
                "lattice_address": question_lattice,
            ...
            "model_state_refs": ordered_models,
            "lattice_refs": [question_lattice],
    ...
    def _validate_run_semantics(self, record: dict[str, Any]) -> None:
        ...
        if not isinstance(question, dict):
            raise StorageError("run question must be an object")
        ...
        expected_address = lattice_address(
            "question",
            question.get("epistemic_role"),
        ...
        if not isinstance(file_ids, list):
            raise StorageError("file_ids must be an array")
        if file_ids != _sorted_refs(file_ids, field="file_ids"):
        ...
        if collection_ref is not None:
            if not isinstance(collection_ref, dict):
                raise StorageError("collection_ref must be an object or null")
        ...
        if not isinstance(model_refs, list):
            raise StorageError("model_state_refs must be an array")
        if model_refs != _sorted_refs(model_refs, field="model_state_refs"):
    ...
    def _validate_event_semantics(self, event: dict[str, Any], *, require_store_refs: bool = True) -> None:
        ...
        if not isinstance(parents, list):
            raise StorageError("parent_event_ids must be an array")
        if parents != _sorted_refs(parents, field="parent_event_ids"):
        ...
        if not isinstance(file_ids, list):
            raise StorageError("event file_ids must be an array")
        if file_ids != _sorted_refs(file_ids, field="file_ids"):
    ...
    def append_event(
        ...
        *,
        ...
        for file_id in ordered_files:
            self.storage.get_file_record(file_id)
        ordered_record_refs = sorted(set(record_refs), key=lambda value: value.encode("utf-8"))
        if any(not isinstance(value, str) or not value for value in ordered_record_refs):
            raise StorageError("record_refs must contain non-empty strings")
        ...
        with self._exclusive_run_lock(run_id):
            ...
            basis = {
                ...
                "lattice_address": address,
                "file_ids": ordered_files,
                "record_refs": ordered_record_refs,
                "payload": payload,
                "authority": "storage-only",
            }
    ...
    def verify_run(self, run_id: str) -> dict[str, Any]:
        ...
        fingerprint = self.fingerprint_run(run_id)
        return {
            "protocol": VERIFICATION_PROTOCOL,
            "status": "valid",
            "run_id": run_id,
            "events": len(events),
            "file_ids": len(run["file_ids"]),
            "collection_snapshot_id": (
                run["collection_ref"]["snapshot_id"] if run["collection_ref"] is not None else None
            ),
    ...
    def fingerprint_run(self, run_id: str) -> dict[str, Any]:
        ...
        inventory = {
            ...
            "event_record_sha256": [
                _sha256_hex(canonical_json_bytes(event)) for event in events
            ],
            "file_ids": run["file_ids"],
            "collection_ref": run["collection_ref"],
            "lattice_profile": run["lattice_profile"],
            "authority": "integrity-not-truth",
        }
    ...
    def import_record_set(self, record_set: dict[str, Any]) -> dict[str, Any]:
        ...
        encoded = canonical_json_bytes(record_set)
        if len(encoded) > MAX_RECORD_SET_BYTES:
        ...
        for index, event in enumerate(ordered):
            ...
            for parent_id in event["parent_event_ids"]:
                parent = next((item for item in ordered if item["event_id"] == parent_id), None)
                if parent is None:
        ...
        with self._exclusive_run_lock(run_id):
            ...
            if run_path.exists():
                ...
                current_event_ids = [event["event_id"] for event in self.list_events(run_id)]
                imported_ids = [event["event_id"] for event in ordered]
                if len(imported_ids) < len(current_event_ids):
                    raise StorageError("import would truncate append-only run history")
                if imported_ids[: len(current_event_ids)] != current_event_ids:
            ...
            head = {
                "run_id": run_id,
                "event_id": ordered[-1]["event_id"] if ordered else None,
