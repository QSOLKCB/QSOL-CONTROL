...
def _source_fingerprint(rows: list[dict[str, Any]]) -> str:
    inventory = {
        ...
        "canonical_entries": sorted(
            rows, key=lambda row: row["logical_path"].encode("utf-8")
        ),
...
def _validate_canonical_store(root: Path) -> tuple[str, dict[str, list[str]]]:
    """Verify canonical CONTROL state without making derived indexes a gate."""
    ...
    if collection_root.exists():
        for directory in sorted(
            (p for p in collection_root.iterdir() if p.is_dir()),
            key=lambda p: p.name.encode("ascii"),
        ):
            ...
            while True:
                ...
                if cursor["revision"] != expected_revision:
                    raise ArkRepositoryBundleError(
                        "collection snapshot revision chain is discontinuous"
                    )
                for file_id in cursor["members"]:
                    record = store.get_file_record(file_id)
                    if PRIVACY_RANK[record["privacy_class"]] > PRIVACY_RANK[privacy]:
                        raise ArkRepositoryBundleError(
                            "collection membership reduces File privacy classification"
                        )
                previous = cursor["previous_snapshot_id"]
                if previous is None:
                    if cursor["revision"] != 0:
                        raise ArkRepositoryBundleError(
                            "collection snapshot lineage terminated before revision 0"
                        )
                    break
                expected_revision -= 1
    ...
    if run_root.exists():
        for path in sorted(run_root.glob("*.json"), key=lambda p: p.name.encode("ascii")):
            if path.is_symlink() or re.fullmatch(r"[0-9a-f]{64}\.json", path.name) is None:
                raise ArkRepositoryBundleError("run registry contains malformed entry")
            interactions.verify_run("sha256:" + path.stem)

    models = ModelStateRegistry(root)
    for state in models.list_states():
    ...
    for replay in replays.list_replays():
...
def _collection_entries(root: Path, package_privacy: str) -> list[tuple[str, bytes, str, str]]:
    ...
    for directory in sorted(
        (p for p in base.iterdir() if p.is_dir()), key=lambda p: p.name.encode("ascii")
    ):
        if directory.is_symlink() or re.fullmatch(r"[0-9a-f]{64}", directory.name) is None:
            raise ArkRepositoryBundleError("collection registry contains malformed directory")
        descriptor, descriptor_bytes = _canonical_json_file(
            directory / "collection.json", "collection descriptor"
        )
        privacy = descriptor.get("privacy_class", package_privacy)
        output.append(
            (
                f"store/records/collections/{directory.name}/collection.json",
                descriptor_bytes,
                "control-collection-record",
                privacy,
            )
        )
        head_value, head_bytes = _canonical_json_file(
            directory / "HEAD.json", "collection HEAD"
        )
        if set(head_value) != {"snapshot_id"}:
            raise ArkRepositoryBundleError("collection HEAD fields are invalid")
        output.append(
            (
                f"store/records/collections/{directory.name}/HEAD.json",
                head_bytes,
                "control-collection-head",
                privacy,
            )
        )
        ...
        if snapshots.exists():
            for path in sorted(snapshots.glob("*.json"), key=lambda p: p.name.encode("ascii")):
                if path.is_symlink() or re.fullmatch(r"[0-9a-f]{64}\.json", path.name) is None:
                    raise ArkRepositoryBundleError(
                        "collection snapshots contain malformed entry"
                    )
                _, data = _canonical_json_file(path, "collection snapshot")
                output.append(
                    (
                        f"store/records/collections/{directory.name}/snapshots/{path.name}",
                        data,
...
def _collect_source(
    root: Path,
) -> tuple[list[tuple[str, bytes, str, str]], str, list[dict[str, Any]]]:
    ...
    if objects.exists():
        for prefix in sorted(
            (p for p in objects.iterdir() if p.is_dir()), key=lambda p: p.name.encode("ascii")
        ):
            ...
            for path in sorted(
                (p for p in prefix.iterdir() if p.is_file()), key=lambda p: p.name.encode("ascii")
            ):
                if (
                    path.is_symlink()
                ...
                source_rows.append(
                    _canonical_row(logical, data, "control-raw-object", privacy)
                )
...
def _schema_entries(repository_root: Path) -> list[tuple[str, bytes, str, str]]:
    ...
    for path in sorted(schema_root.glob("*.json"), key=lambda p: p.name.encode("utf-8")):
        data = _read_regular(path, f"schema/{path.name}")
    ...
    *,
    ...
    if include_dna:
        if type(dna_max_file_bytes) is not int or dna_max_file_bytes < 0:
            raise ArkRepositoryBundleError(
                "dna_max_file_bytes must be a non-negative integer"
            )
        file_root = root / "records" / "files"
        if file_root.exists():
            for path in sorted(file_root.glob("*.json"), key=lambda p: p.name.encode("ascii")):
...
def _chunk_entries(entries: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
    ...
    for entry in entries:
        ...
        if size > MAX_CAPSULE_PAYLOAD_BYTES:
            raise ArkRepositoryBundleError(
                "single transport entry exceeds Phase 8 capsule payload budget"
            )
        if current and (
...
def build_repository_recovery_package(
    ...
    *,
    ...
    if destination.exists() or destination.is_symlink():
        raise ArkRepositoryBundleError(
            "recovery package output must not already exist"
        )
...
def _load_bootstrap(package: Path) -> tuple[dict[str, Any], bytes]:
    ...
    if value.get("protocol") != PROTOCOL or value.get("container") != CONTAINER:
        raise ArkRepositoryBundleError(
            "repository recovery bootstrap protocol/container mismatch"
        )
    ...
    if (
        value.get("requires_webui") is not False
        or value.get("requires_original_search_engine") is not False
    ):
        raise ArkRepositoryBundleError(
            "repository recovery must remain independent of WebUI/search engine"
        )
...
def _capsule_entries(
    package: Path, bootstrap: dict[str, Any]
    ...
    for index, row in enumerate(rows):
        ...
        if report["entry_count"] != row.get("entry_count"):
            raise ArkRepositoryBundleError(
                "repository recovery capsule entry count mismatch"
            )
        for entry in entries:
            logical = _safe_relative(entry["logical_path"])
            if logical in by_path:
                raise ArkRepositoryBundleError(
                    "duplicate logical path across recovery capsules"
                )
...
def _reconstruct_large_objects(
    target: Path, entries: dict[str, dict[str, Any]]
) -> None:
    manifest_paths = sorted(
        (
            path
            for path in entries
            if path.startswith("transport/large-objects/")
    ...
    for manifest_path in manifest_paths:
        ...
        if not target_logical.startswith("store/objects/sha256/"):
            raise ArkRepositoryBundleError(
                "large-object target is outside canonical object store"
            )
        ...
        if destination.exists() or destination.is_symlink():
            raise ArkRepositoryBundleError(
                "large-object recovery target already exists"
            )
        ...
        with destination.open("xb") as handle:
            for index, row in enumerate(chunks):
                expected = (
                    f"transport/large-objects/{manifest['sha256']}/{index:08d}.bin"
                )
                if not isinstance(row, dict) or row.get("path") != expected:
                    raise ArkRepositoryBundleError(
                        "large-object chunk ordering is invalid"
                    )
                ...
                data = entry["data"]
                if (
                    sha256_hex(data) != row.get("sha256")
                    or len(data) != row.get("size_bytes")
                ):
                    raise ArkRepositoryBundleError(
                        "large-object chunk hash/size mismatch"
                    )
                handle.write(data)
                digest.update(data)
                size += len(data)
        if (
            digest.hexdigest() != manifest.get("sha256")
            or size != manifest.get("size_bytes")
        ):
            raise ArkRepositoryBundleError(
                "reassembled large object failed source hash/size verification"
            )


def _verify_restored_store(store_root: Path) -> dict[str, Any]:
    # The reconstructed store intentionally contains no canonical derived indexes,
    # so the ordinary storage verifier can now validate the complete canonical core.
...
def _reconstruct(
    package: Path, target: Path, *, require_absent: bool = True
) -> dict[str, Any]:
    ...
    for logical in sorted(entries, key=lambda value: value.encode("utf-8")):
        ...
        if not logical.startswith(("store/", "schemas/", "lattice/", "optional/")):
            raise ArkRepositoryBundleError(
                "recovery capsule contains unsupported logical path"
            )
    ...
    if fingerprint != bootstrap.get("source_fingerprint"):
        raise ArkRepositoryBundleError(
            "reconstructed canonical source fingerprint mismatch"
        )
    if len(rows) != bootstrap.get("canonical_entry_count"):
        raise ArkRepositoryBundleError(
            "reconstructed canonical entry count mismatch"
        )
    ...
    if (target / "store" / "records" / "indexes").exists() and any(
        (target / "store" / "records" / "indexes").iterdir()
    ):
        raise ArkRepositoryBundleError(
            "derived indexes entered canonical reconstructed store"
        )
    return {
        "protocol": PROTOCOL,
...
def verify_repository_recovery_package(package: str | Path) -> dict[str, Any]:
    ...
    if root.is_symlink() or not root.is_dir():
        raise ArkRepositoryBundleError(
            "repository recovery package must be a non-symlink directory"
        )
    ...
    if actual_root != expected_root:
        raise ArkRepositoryBundleError(
            "repository recovery package contains unexpected top-level entries"
        )
    expected_capsules = {Path(row["path"]).name for row in bootstrap["capsules"]}
    actual_capsules = {path.name for path in (root / "capsules").iterdir()}
    if actual_capsules != expected_capsules:
        raise ArkRepositoryBundleError(
            "repository recovery package capsule set differs from bootstrap"
        )
...
def restore_repository_recovery_package(
    package: str | Path, target: str | Path
) -> dict[str, Any]:
    package_root = Path(package)
    target_root = Path(target)
    if target_root.exists() or target_root.is_symlink():
        raise ArkRepositoryBundleError(
            "repository recovery target must not already exist"
        )
    parent = target_root.parent
    parent.mkdir(parents=True, exist_ok=True)
    staging = Path(
        tempfile.mkdtemp(prefix=f".{target_root.name}.restore-", dir=parent)
    )
    try:
