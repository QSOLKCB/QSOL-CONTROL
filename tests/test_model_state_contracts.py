...
class ModelStateContractTests(unittest.TestCase):
    ...
    def test_manifest_registers_phase4_surfaces_without_freezing_later_phases(self):
        manifest = self.load("manifest.json")
        major = int(manifest["schema_version"].split(".", 1)[0])
        self.assertEqual(major, 2)
        self.assertEqual(manifest["model_state_contract"], "ai/model-state-contract.json")
        self.assertEqual(manifest["interfaces"]["model_state_cli"], "tools/model_state.py")
        self.assertEqual(manifest["persistent_storage"]["model_state_runtime"], "storage/model_state_registry.py")
        self.assertEqual(manifest["schemas"]["model_state_comparison"], "schema/model-state-comparison.schema.json")
        self.assertEqual(manifest["schemas"]["model_state_run_comparison"], "schema/model-state-run-comparison.schema.json")
        self.assertEqual(manifest["schemas"]["model_state_archaeology"], "schema/model-state-archaeology.schema.json")
        self.assertGreaterEqual(manifest["status"]["completed_through_roadmap_phase"], 4)
        self.assertEqual(manifest["status"]["model_state_registry"], "implemented-phase-4")
    ...
    def test_model_state_schema_enforces_epistemic_boundary(self):
        ...
        self.assertEqual(props["epistemic_boundary"]["const"], "MODEL_STATE != MODEL_MIND")
        self.assertFalse(props["hidden_chain_of_thought_captured"]["const"])
        self.assertFalse(props["model_mind_captured"]["const"])
        self.assertEqual(props["authority"]["const"], "reproducibility-metadata-only")
        ...
        self.assertEqual(provenance["properties"]["system.control_run_id"]["const"], "locally_verified")
        self.assertEqual(provenance["properties"]["captured_at"]["const"], "observed")
    ...
    def test_archaeology_schema_forbids_mind_and_artifact_byte_claims(self):
        ...
        self.assertEqual(props["ui_boundary_label"]["const"], "Reproducibility metadata — not model mind")
    ...
    def test_machine_contract_pins_ui_labels_privacy_and_verification_authority(self):
        ...
        self.assertEqual(provenance["locally_verified_assignment"], "runtime-only-after-CONTROL-verification")
    ...
    def test_roadmap_phase4_remains_complete_and_webui_preserves_labels(self):
        roadmap = (ROOT / "ROADMAP.md").read_text(encoding="utf-8")
        phase4 = roadmap.split("## Phase 4 — AI model-state registry", 1)[1].split("## Phase 5 — Human WebUI", 1)[0]
