...
class ReplayContractTests(unittest.TestCase):
    ...
    def test_replay_contract_pins_immutability_and_legacy_honesty(self):
        ...
        self.assertTrue(contract["original_result_policy"]["before_after_integrity_fingerprint_required"])
        self.assertFalse(contract["original_result_policy"]["original_run_record_rewritten"])
        self.assertFalse(contract["original_result_policy"]["original_event_chain_rewritten"])
    ...
    def test_replay_schemas_pin_nonclaims(self):
        ...
        self.assertEqual(replay["properties"]["protocol"]["const"], REPLAY_RECORD_PROTOCOL)
        ...
        self.assertEqual(report["properties"]["protocol"]["const"], REPLAY_REPORT_PROTOCOL)
        self.assertFalse(report["properties"]["comparison_is_truth"]["const"])
        self.assertFalse(
            report["properties"]["model_state_comparison_is_mind_comparison"]["const"]
        )
        self.assertEqual(timeline["properties"]["protocol"]["const"], RESEARCH_TIMELINE_PROTOCOL)
    ...
    def test_phase7_roadmap_is_complete_and_phase8_remains_partial(self):
        roadmap = (ROOT / "ROADMAP.md").read_text(encoding="utf-8")
        phase7 = roadmap.split("## Phase 7 — Replay and longitudinal research", 1)[1].split(
            "## Phase 8 — ARK recovery bridge", 1
        )[0]
    ...
    def test_manifest_and_bootstrap_register_phase7(self):
        ...
        self.assertEqual(manifest["status"]["completed_through_roadmap_phase"], 7)
