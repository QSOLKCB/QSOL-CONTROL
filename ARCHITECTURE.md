...
Ownership of storage mechanics does not confer truth authority. Invocation authority does not confer authority to rewrite the invoked system. Recording model/runtime metadata does not confer access to hidden cognition. Replay changes neither rule.
...
Replay POST requests use the existing loopback, session-token, and same-origin mutation boundary. Phase 7 creates no remote service.

### Machine surface

The machine interface remains `qsol-control-agent-api/1` over bounded JSONL/stdio. Phase 7 extends its exact operation catalogue with:
...
api/dispatcher.py                 machine replay operation dispatch + quotas
...
See `README.md`, `README4AI.md`, `docs/REPLAY.md`, `docs/AGENT-API.md`, `AGENTS.md`, `SECURITY.md`, and `ROADMAP.md`.
