...
def _strict_json_object(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise ConsensusAdapterError(f"{label} returned invalid JSON") from exc
    if not isinstance(value, dict):
        raise ConsensusAdapterError(f"{label} JSON root must be an object")
    return value
...
def _validate_command(command: Sequence[str]) -> tuple[str, ...]:
    if (
        ...
        raise ConsensusAdapterError(
            "consensus command must be a bounded non-empty argv sequence"
        )
    out = []
    for arg in command:
        if (
...
def validate_intent(intent: Any) -> dict[str, Any]:
    """Fully validate an externally supplied intent, not only its content hash."""

    if not isinstance(intent, dict):
        raise ConsensusAdapterError("consensus intent must be an object")
    required = {
        "intent_id",
        "protocol",
        "operation",
    ...
    if set(intent) != required or intent.get("protocol") != INTENT_PROTOCOL:
        raise ConsensusAdapterError("consensus intent protocol/field set mismatch")
    operation = intent.get("operation")
    if operation not in MUTATION_OPERATIONS:
        raise ConsensusAdapterError(
            "consensus intent operation must be a known CONTROL mutation"
        )
    params = intent.get("params")
    if not isinstance(params, dict):
        raise ConsensusAdapterError("consensus intent params must be an object")
    try:
        reject_forbidden_fields(params)
    except AgentAPIError as exc:
        raise ConsensusAdapterError(
            "consensus intent contains forbidden authority/credential fields"
        ) from exc
    fingerprint = intent.get("expected_store_fingerprint")
    if not isinstance(fingerprint, str) or SHA256_REF.fullmatch(fingerprint) is None:
        raise ConsensusAdapterError(
            "expected_store_fingerprint must be a sha256: reference"
        )
    if intent.get("authority") != "coordination-only":
        raise ConsensusAdapterError("consensus intent authority must remain coordination-only")
    if intent.get("semantic_authority_claimed") is not False:
        raise ConsensusAdapterError("consensus intent attempted semantic authority escalation")
    claimed = intent.get("intent_id")
    if not isinstance(claimed, str) or SHA256_REF.fullmatch(claimed) is None:
        raise ConsensusAdapterError("consensus intent_id is invalid")
    payload = {key: value for key, value in intent.items() if key != "intent_id"}
    if claimed != sha256_ref(canonical_json_bytes(payload)):
        raise ConsensusAdapterError("consensus intent identity mismatch")
    return intent


def build_intent(
    *, operation: str, params: dict[str, Any], expected_store_fingerprint: str
) -> dict[str, Any]:
    payload = {
    ...
    intent = {"intent_id": sha256_ref(canonical_json_bytes(payload)), **payload}
...
def validate_receipt(
    ...
    *,
    ...
    if (
        ...
        and receipt["state_fingerprint"] != expected_state_fingerprint
    ):
        raise ConsensusAdapterError(
            "consensus receipt state fingerprint does not match intended pre-state"
        )
    ...
    if (
        ...
        or type(observed_votes) is not int
        or required_votes < 1
        or observed_votes < required_votes
    ):
    ...
    if (
        receipt["authority"] != "coordination-only"
        or receipt["semantic_authority_claimed"] is not False
    ):
...
def _bounded_process(
    ...
    *,
    ...
    """Run a provider while capping stdout/stderr during, not after, production."""
    ...
    def drain(stream, target: bytearray, limit: int, label: str) -> None:
        try:
            while True:
    ...
    threads = [
        ...
        threading.Thread(
            target=drain,
            args=(proc.stdout, stdout, MAX_RESPONSE_BYTES, "stdout"),
            daemon=True,
        ),
        threading.Thread(
            target=drain,
            args=(proc.stderr, stderr, MAX_STDERR_BYTES, "stderr"),
            daemon=True,
        ),
    ]
    ...
    try:
        ...
        proc.kill()
        proc.wait()
        raise ConsensusAdapterError("external consensus provider timed out") from exc
    finally:
        for thread in threads:
    ...
    if overflow:
        raise ConsensusAdapterError(
            f"consensus provider {overflow[0]} exceeds byte limit"
        )
    if thread_errors:
        raise ConsensusAdapterError("external consensus provider pipe failure")
...
class ExternalConsensusAdapter:
    ...
    def _call(self, operation: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        response = _strict_json_object(stdout, "external consensus provider")
        if (
            set(response) != {"protocol", "ok", "result"}
            or response.get("protocol") != PROVIDER_RESPONSE_PROTOCOL
            or response.get("ok") is not True
        ):
            raise ConsensusAdapterError(
                "external consensus provider response contract mismatch"
            )
    ...
    def health(self) -> dict[str, Any]:
        ...
        if result.get("status") != "ok" or not isinstance(
            result.get("provider_protocol"), str
        ):
    ...
    def propose(self, intent: dict[str, Any]) -> dict[str, Any]:
        ...
        expected_state = validated["expected_store_fingerprint"]
        proposed = self._call("commit.propose", validated)
        ...
        if (
            set(verification) != {"verified", "intent_id"}
            or verification.get("verified") is not True
            or verification.get("intent_id") != intent_id
        ):
            raise ConsensusAdapterError(
                "external consensus receipt post-verification failed"
            )
        return receipt
    ...
    def verify(self, receipt: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            set(result) != {"verified", "intent_id"}
            or result.get("verified") is not True
            or result.get("intent_id") != local["intent_id"]
        ):
